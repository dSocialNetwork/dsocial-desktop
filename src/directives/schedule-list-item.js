export default () => {
  return {
    ...
    `,
    controller: ($scope, $rootScope, $location, $filter, $window, dSocialService, activeUsername) => {
      ...
      $scope.deleteClicked = (schedule) => {
        if ($window.confirm($filter('translate')('ARE_YOU_SURE'))) {
          $scope.deleting = true;
          dSocialService.removeSchedule(schedule._id, activeUsername()).then((resp) => {
      ...
      $scope.moveClicked = (schedule) => {
        $scope.moving = true;
        dSocialService.moveSchedule(schedule._id, activeUsername()).then((resp) => {
