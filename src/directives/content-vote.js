export default () => {
  return {
    ...
    controller: ($scope, $rootScope, $timeout, $filter, $uibModal, dpayService, dpayAuthenticatedService, helperService, activeUsername) => {
      ...
      const estimate = (w) => {
        const vestToBp = (vests) => {
          return vests / 1e6 * $rootScope.dpayPerMVests;
        };

        const bpToVests = (bp) => {
          return bp * 1e6 / $rootScope.dpayPerMVests;
        };

        const vestsToRshares = (bp, voting_power, vote_pct) => {
          const vestingShares = parseInt(bpToVests(bp) * 1e6, 10);
        ...
        const bp = vestToBp(totalVests);

        const votePct = w * 100;

        const rShares = vestsToRshares(bp, votingPower, votePct);
