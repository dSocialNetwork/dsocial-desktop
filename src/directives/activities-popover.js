export default () => {
  return {
    ...
    controller: ($scope, $rootScope, $location, dpayService, dSocialService, activeUsername) => {
      ...
      const loadActivities = () => {
        ...
        switch ($rootScope.activityType.selected) {
          case 'votes':
            prms = dSocialService.getMyVotes(account);
            break;
          case 'replies':
            prms = dSocialService.getMyReplies(account);
            break;
          case 'mentions':
            prms = dSocialService.getMyMentions(account);
            break;
          case 'follows':
            prms = dSocialService.getMyFollows(account);
            break;
          case 'reblogs':
            prms = dSocialService.getMyReblogs(account);
            break;
          default:
            prms = dSocialService.getActivities(account);
        }
