...
export default () => {
  return {
    ...
    controller: ($scope, $rootScope, $timeout, dSocialService, activeUsername) => {
      ...
      $scope.uploadFiles = async (files) => {
        ...
        for (let f of files) {
          ...
          await dSocialService.uploadImage(f, (e) => {
        ...
        for (let u of uploadedFiles) {
          dSocialService.addMyImage(activeUsername(), u).then((resp) => {
            // console.log(resp)
          }).catch((e) => {
            // console.log(e)
          });
