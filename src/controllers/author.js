export default ($scope, $rootScope, $routeParams, $timeout, $q, $location, $window, $uibModal, $filter, dpayService, dSocialService, dpayAuthenticatedService, activeUsername, constants) => {
  ...
  const loadAccount = async (refresh = false) => {
    ...
    dSocialService.getActiveVotes(username).then((resp) => {
  ...
  const loadVisitor = async (refresh = false) => {
    ...
    dSocialService.isFavorite(visitorName, username).then((resp) => {
  ...
  const loadTopPosts = () => {
    dSocialService.getTopPosts(username).then((resp) => {
  ...
  $scope.fav = () => {
    ...
    dSocialService.addFavorite(activeUsername(), username).then((resp) => {
  ...
  $scope.unFav = () => {
    $scope.vFavoriting = true;
    dSocialService.removeFavoriteUser(activeUsername(), username).then((resp) => {
