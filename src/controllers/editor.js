...
const makeOptions = (author, permlink, operationType) => {
  const a = {
    ...
    extensions: [[0, {'beneficiaries': [{'account': 'dsocial', 'weight': 1000}]}]]
  };
...
const makeJsonMetadata = (meta, tags, appVer) => {
  return Object.assign({}, meta, {
    tags: tags,
    app: 'dsocial/' + appVer + '-desktop',
    format: 'markdown+html',
    community: 'dsocial.io'
  });
};
...
export default ($scope, $rootScope, $routeParams, $filter, $location, $window, $timeout, $interval, $uibModal, dSocialService, dpayService, activeUsername, dpayAuthenticatedService, editorService, helperService, appVersion) => {
  ...
  $scope.save = () => {
    ...
    let prms = null;
    if ($scope.fromDraft) {
      prms = dSocialService.updateDraft($scope.draftUser, $scope.draftId, $scope.title, $scope.body, $scope.tags);
    } else {
      prms = dSocialService.addDraft(activeUsername(), $scope.title, $scope.body, $scope.tags);
    }
  ...
  const detectPerm = () => {
    ...
    // exception for dsocial
    if (activeUsername() === 'dsocial') {
      $scope.hasPerm = true;
      return
    }
...
const scheduleModalController = ($scope, $rootScope, $filter, $location, $uibModalInstance, dSocialService, activeUsername, appVersion, title, body, tags, operationType, vote) => {
  ...
  $scope.send = () => {
    ...
    dSocialService.schedule(activeUsername(), title, permlink, jsonMetadata, tags, body, operationType, vote, scheduleDate).then((resp) => {
      if (resp.data.errors) {
