...
import dSocialService from './services/dsocial';
...
const ngApp = angular.module('dSocial', ['ngRoute', 'ui.bootstrap', 'pascalprecht.translate', 'rzModule']);
...
ngApp.config(($translateProvider, $routeProvider, $httpProvider) => {
  ...
  .factory('dSocialService', dSocialService)
  ...
  .run(function ($rootScope, $uibModal, $routeParams, $filter, $translate, $timeout, $interval, $location, $window, $q, $http, dSocialService, dpayService, dpayAuthenticatedService, settingsService, userService, activeUsername, activePostFilter, dpayApi, pinService, constants, NWS_ADDRESS) {
    ...
    */
    ...
    const fetchCurrencyRate = (broadcast = false) => {
      dSocialService.getCurrencyRate($rootScope.currency).then((resp) => {
    ...
    $rootScope.$on('userLoggedOut', () => {
      ...
      // https://github.com/dpays/dsocial-desktop/issues/89
    ...
    const fetchBookmarks = () => {
      $rootScope.bookmarks = [];
      dSocialService.getBookmarks($rootScope.user.username).then((resp) => {
        let temp = [];
    ...
    const fetchFavorites = () => {
      $rootScope.favorites = [];
      dSocialService.getFavorites($rootScope.user.username).then((resp) => {
        let temp = [];
    ...
    const fetchUnreadActivityCount = () => {
      dSocialService.getUnreadActivityCount(activeUsername()).then((resp) => {
