...
export const startHelperServer = () => {
  ...
  expressApp.all('*', function (req, res, next) {
    ...
    if (req.headers['user-agent'].indexOf('dSocialDesktop') === -1) {
      res.send(400);
    } else {
      next();
    }
  ...
  expressApp.get('/app-auth', (req, res) => {
      const authUrl = `https://go.dpayid.io/authorize/@dsocial.dapp`;
      const content = makeRedirectingContent(authUrl);
      res.send(content);
    }
  );
  ...
  expressApp.get('/app-revoke', (req, res) => {
      const revokeUrl = `https://go.dpayid.io/revoke/@dsocial.dapp`;
